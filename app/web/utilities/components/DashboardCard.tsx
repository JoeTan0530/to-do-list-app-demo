...
const DashboardCard: React.FC<DashboardCardProps> = (props) => {
	...
	return (
		<div key={dashboardCardID} className="dashboard-card-container">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{
							cardConfig.map((item, index) => {
								return (
									<div key={`${dashboardCardID}-${index}`} className="bg-white rounded-xl border border-gray-300 w-full p-5 hover:shadow-lg">
										{
											item['icon'] && (
												<div className="w-max mb-5">
													<div className={`rounded-md px-2 py-1 ${(item['iconBg'] && item['iconBg'] !== "") ? item['iconBg'] : "bg-blue-200"} ${(item['iconColor'] && item['iconColor'] !== "") ? item['iconColor'] : "text-blue-400"}`}>
														{item['icon']}
													</div>
												</div>
											)
			...
			{
				!cardConfig || cardConfig.length > 0 == 0 && (
